...
def exchange_rate_evolution(request, base=getattr(settings, 'DEFAULT_BASE_CURRENCY'), target='USD', months=6):
    ...
    rates = CurrencyExchangeRate.objects.all().values_list('valuation_date', 'rate_value')
    import ipdb; ipdb.set_trace()
    base_rate = rates.filter(source_currency__code=base).first()
    # data = EXAMPLE_DATA
    data = [[str(rate[0]), float(rate[1])] for rate in rates]
    schema = CHART_SCHEMA

    fusion_table = FusionTable(schema, data)
    time_series = TimeSeries(fusion_table)

    time_series.AddAttribute("caption", """{
                                        text: 'Exchange Rate'
                                        }""")

    time_series.AddAttribute("subcaption", """{
                                    text: '""" + base + """'
                                    }""")
    #                                     text: """ + base + """
    ...
    fc_chart = FusionCharts("timeseries", "ex1", 700, 450, "chart-1", "json", time_series)
